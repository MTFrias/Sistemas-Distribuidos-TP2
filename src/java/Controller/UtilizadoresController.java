...
public class UtilizadoresController implements Serializable {
    ...
    public void modificarCategoriaUtilizador(int id, int categoria){
        Utilizadores u = utili.find(id);
        if(u != null){
            if(u.getIdTipo() == 1){
                u.setIdTipo(2);
            }else{
    ...
    public void addUser(){ 
        if(validations() && validationEmail()){
    ...
    public void login(){ 
        ...
        for(Utilizadores item: UserList){
            if(item.getEmail().equals(email) && item.getPassword().equals(PBKDF2.hashPassword("sha512", password, 1000, 512))){
                ...
                userSession.setCategoria(item.getIdTipo());
